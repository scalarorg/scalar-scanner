...
export const getInputType = (string, chainsData) => {
  const regexMap = {
    ...
    validator: new RegExp('scalarvaloper.*$', 'igm'),
    scalarAddress: new RegExp('scalar.*$', 'igm'),
    cosmosAddress: Object.fromEntries(toArray(chainsData).filter(d => d.id !== 'scalarnet' && d.prefix_address).map(d => [d.id, new RegExp(`${d.prefix_address}.*$`, 'igm')])),
  }
